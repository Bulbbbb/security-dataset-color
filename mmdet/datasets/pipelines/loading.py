...
# FIXME：将加载训练图片的代码修改为加载边缘图
@PIPELINES.register_module
class LoadEdgeMapFromFile(object):
    """加载边缘图，用于边缘增强"""

    def __init__(self, to_float32=False, color_type='grayscale'):
        self.to_float32 = to_float32
        self.color_type = color_type

    def __call__(self, results):
        if results['edge_map_prefix'] is not None:
            filename = osp.join(
                results['edge_map_prefix'],
                results['img_info']['filename'].replace(".png", ".jpg"))
        else:
            raise ValueError("请设置“edge_map_prefix”")
        img = mmcv.imread(filename, self.color_type)
        if self.to_float32:
            img = img.astype(np.float32)

        results['edge_map_filename'] = filename
        results['edge_map'] = img
        results['edge_map_shape'] = img.shape
        results['edge_map_ori_shape'] = img.shape
        # Set initial values for default meta_keys
        results['edge_map_pad_shape'] = img.shape
        results['edge_map_scale_factor'] = 1.0
        num_channels = 1 if len(img.shape) < 3 else img.shape[2]
        results['edge_map_norm_cfg'] = dict(mean=np.zeros(num_channels,
                                                          dtype=np.float32),
                                            std=np.ones(num_channels,
                                                        dtype=np.float32),
                                            to_rgb=False)
