...
@PIPELINES.register_module
class BGFormatBundle(DefaultFormatBundle):
    def __init__(self, **kwargs):
        super(DefaultFormatBundle, self).__init__(**kwargs)
    ...
    def __call__(self, results):
        ...
        if 'edge_map' in results:
            ...
            if len(edge_map.shape) < 3:
                edge_map = np.expand_dims(edge_map, -1)
            edge_map = np.ascontiguousarray(edge_map.transpose(2, 0, 1))

            edge_map = np.ascontiguousarray(edge_map)
            results['edge_map'] = DC(to_tensor(edge_map), stack=True, cpu_only=False)
            # results['edge_map'] = DC(to_tensor(edge_map), cpu_only=True)
        for key in ['proposals', 'gt_bboxes', 'gt_bboxes_ignore', 'gt_labels']:
            if key not in results:
                continue
            results[key] = DC(to_tensor(results[key]))
