...
@PIPELINES.register_module
class BGResize(Resize):
    def __init__(self, **kwargs):
        super(BGResize, self).__init__(**kwargs)

    def _resize_edge_img(self, results):
        if self.keep_ratio:
            edge_map, scale_factor = mmcv.imrescale(
                results['edge_map'], results['scale'], return_scale=True)
        else:
            edge_map, w_scale, h_scale = mmcv.imresize(
                results['edge_map'], results['scale'], return_scale=True)
            scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                    dtype=np.float32)
        results['edge_map'] = edge_map
        results['edge_map_shape'] = edge_map.shape
        results['edge_map_pad_shape'] = edge_map.shape  # in case that there is no padding
        results['edge_map_scale_factor'] = scale_factor
        results['edge_map_keep_ratio'] = self.keep_ratio

    def __call__(self, results):
        if 'scale' not in results:
            self._random_scale(results)
        self._resize_img(results)
        self._resize_edge_img(results)
        self._resize_bboxes(results)
        self._resize_masks(results)
        self._resize_seg(results)
        return results
...
@PIPELINES.register_module
class BGRandomFlip(RandomFlip):
    def __init__(self, **kwargs):
        super(BGRandomFlip, self).__init__(**kwargs)
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            results['img'] = mmcv.imflip(
                results['img'], direction=results['flip_direction'])
            # flip edge_map
            results['edge_map'] = mmcv.imflip(
                results['edge_map'], direction=results['flip_direction'])
            # flip bboxes
            for key in results.get('bbox_fields', []):
                results[key] = self.bbox_flip(results[key],
                                              results['img_shape'],
...
class BGPad(Pad):
    def __init__(self, **kwargs):
        super(BGPad, self).__init__(**kwargs)

    def _pad_edge_map(self, results):
        if self.size is not None:
            padded_edge_map = mmcv.impad(results['edge_map'], self.size, self.pad_val)
        elif self.size_divisor is not None:
            padded_edge_map = mmcv.impad_to_multiple(
                results['edge_map'], self.size_divisor, pad_val=self.pad_val)
        results['edge_map'] = padded_edge_map
        results['edge_map_pad_shape'] = padded_edge_map.shape
        results['edge_map_pad_fixed_size'] = self.size
        results['edge_map_pad_size_divisor'] = self.size_divisor

    def __call__(self, results):
        self._pad_img(results)
        self._pad_edge_map(results)
        self._pad_masks(results)
        self._pad_seg(results)
        return results
